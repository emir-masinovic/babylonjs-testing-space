window.addEventListener("DOMContentLoaded", async function () {
  ...
  let createScene = async function () {
    ...
    shape = gridBox; //For setting boundaries in render loop
    ...
    let gridPlaneMaterial = new BABYLON.GridMaterial(
      "gridPlaneMaterial",
      scene
    );
    ...
    for (let i = 0; i < maxCount; i++) {
      ...
      gridClone.physicsImpostor = new BABYLON.PhysicsImpostor(
        gridClone,
        BABYLON.PhysicsImpostor.BoxImpostor,
        {
          mass: scale,
    ...
    let translate = function (mesh, direction, power) {
      ...
      mesh.physicsImpostor.setLinearVelocity(
        mesh.physicsImpostor
          .getLinearVelocity()
          .add(transformForce(mesh, direction.scale(power)))
      );
    };
  ...
  function switchCam(cameraType, arcCamera, freeCamera) {
    ...
    if (cameraType === "free camera") arcCamera.detachControl(canvas);
    // arcCamera.inputs.remove(arcCamera.inputs.attached.keyboard);
